...
public class addDeptWithMultipleLocations extends LoginPageSetup {
	...
	public void DeptWithLocation() throws InterruptedException
	{
		try
		{
		 driver.get(prop.getProperty("url"));
		    ...
		    and.sendShortName().sendKeys("testdeptmult");
		    wait.until(ExpectedConditions.visibilityOfElementLocated(departmentName));
		    and.sendDepartmentName().sendKeys("testdeptmult");
			...
			for(WebElement listlocation:location)
			{
				...
				if(listlocation.getText().equalsIgnoreCase("Location 3"))
				{
					listlocation.click();
					//break;
				}
