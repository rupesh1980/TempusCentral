...
public class addSameParentDept extends LoginPageSetup {
	...
	public void deleteDepartment() throws InterruptedException
	{
		try
		{
		 driver.get(prop.getProperty("url"));
			...
			{
				if(selectLocation.getText().contains("Dpt"))
				{
			...
			fb.clickAdd().click();
			boolean message=driver.findElement(By.xpath("//*[contains(text(),\'Parent department code must be different\')]")).isDisplayed();
			if(true)
			{
				System.out.println("Parent department code must be differe");
			}
			else
			{
