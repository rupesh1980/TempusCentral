...
public class deleteNewDropDownField extends  LoginPageSetup{
		...
		try
		{
			 driver.get(prop.getProperty("url"));
				...
				Thread.sleep(2000);
		     	...
		     	{
		     		System.out.println(driver.findElements(By.xpath("//div[@style=\'display:flex;padding: 0px 10px;\']/label")).get(i).getText());
		     		if(textfields.get(i).getText().equalsIgnoreCase("Mydropdown new"))
		     		{
		     			driver.findElements(By.xpath("//img[@style=\'width:18px;height:18px;margin-right: -30px;margin-top:-10px;margin-left:-10px;cursor:pointer\']")).get(i).click();
		     	...
		     	{
		     		if(textfields1.get(i).getText().equalsIgnoreCase("Mydropdown new"))
		     		{
